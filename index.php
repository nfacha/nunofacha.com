<!DOCTYPE html>
<html data-animation="11" data-random-animation="true" lang="en">
<head>
    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-60597198-1"></script>
    <script>
        window.dataLayer = window.dataLayer || [];

        function gtag() {
            dataLayer.push(arguments);
        }

        gtag('js', new Date());

        gtag('config', 'UA-60597198-1');
    </script>

    <!--Meta Tags-->
    <meta charset="utf-8">
    <meta content="IE=edge" http-equiv="X-UA-Compatible"/>
    <meta content="width=device-width, initial-scale=1" name="viewport"/>
    <meta content="vcard, resume, personal, portfolio, cv, card, responsive, nuno facha" name="keywords"/>
    <meta content="Nuno Facha - Software Developer from Azores, Portugal" name="description"/>

    <!--Page Title-->
    <title>Nuno Facha - Software Developer</title>

    <!--Plugins Css-->
    <link href="css/plugins.css" rel="stylesheet">
    <!--Main Styles Css-->
    <link href="css/style-dark.css" rel="stylesheet">
    <!--Color Css-->
    <link href="css/blue-color.css" rel="stylesheet">

    <!--Modernizr Js-->
    <script src="js/modernizr.js"></script>

    <!--Favicons-->
    <link href="img/favicon.ico" rel="shortcut icon" type="image/x-icon">

</head>

<body>

<!--Preloader Start-->
<div class="preloader">
    <div class="loader">
        <!--Your Name-->
        <h4>Nuno Facha</h4>
        <span></span>
        <span></span>
        <span></span>
        <span></span>
        <span></span>
        <span></span>
        <span></span>
    </div>
</div>
<!--Preloader End-->

<div id="page">

    <!--Header Start-->
    <header>
        <div class="header-content">

            <!--Mobile Header-->
            <div class="header-mobile">
                <a class="header-toggle"><i class="fas fa-bars"></i></a>
                <h2>Nuno Facha</h2>
            </div>

            <!--Main Header-->
            <div class="header-main" data-simplebar>
                <div class="image-container">
                    <h2 class="header-name">Nuno Facha</h2>
                    <img alt="profile-pic" src="img/profile-img.jpg">
                </div>

                <!--Nav Menus-->
                <nav class="nav-menu">
                    <ul>
                        <li><a class="pt-link active" href="#home"><span class="nav-menu-icon"><i
                                            class="lnr lnr-home"></i></span>Home </a></li>
                        <li><a class="pt-link" href="#about"><span class="nav-menu-icon"><i
                                            class="lnr lnr-user"></i></span>About Me</a></li>
                        <li><a class="pt-link" href="#resume"><span class="nav-menu-icon"><i
                                            class="lnr lnr-license"></i></span>Resume</a></li>
                        <li><a class="pt-link" href="#events"><span class="nav-menu-icon"><i
                                            class="lnr lnr-star"></i></span>Events</a></li>
                        <li><a class="pt-link" href="#stats"><span class="nav-menu-icon"><i
                                            class="lnr lnr-clock"></i></span>Stats</a></li>
                        <li><a class="pt-link" href="https://sourcerer.io/nunofacha" target="_blank"><span
                                        class="nav-menu-icon"><i
                                            class="lnr lnr-rocket"></i></span>Insights</a></li>
                        <!--                        <li><a href="#portfolio" class="pt-link"><span class="nav-menu-icon"><i-->
                        <!--                                class="lnr lnr-briefcase"></i></span>Portfolio</a></li>-->
                        <!--                        <li><a href="#contact" class="pt-link"><span class="nav-menu-icon"><i-->
                        <!--                                class="lnr lnr-envelope"></i></span>Contact</a></li>-->
                    </ul>
                </nav>

                <!--Nav Footer-->
                <div class="nav-footer">
                    <!--Social Links-->
                    <ul class="social">
                        <li><a href="https://www.facebook.com/nuno.facha" target="_blank"><i
                                        class="fab fa-facebook-square"></i></a>
                        </li>
                        <li><a href="https://www.facebook.com/nuno.facha" target="_blank"><i
                                        class="fab fa-twitter-square"></i></a></li>
                        <li><a href="https://www.linkedin.com/in/nunofacha/ target=" _blank""><i
                                    class="fab fa-linkedin"></i></a></li>
                        <li><a href="https://github.com/nfacha" target="_blank"><i class="fab fa-github"></i></a></li>
                    </ul>
                    <!--Copyright Text-->
                    <div class="copy">
                        <p>2020 &copy; Nuno Facha.<br>All Right Reserved.</p>
                    </div>
                </div>

            </div>
        </div>
    </header>
    <!--Header End-->


    <!--Main Start-->
    <div class="site-main" id="main">


        <!--Banner Section Start-->
        <section class="banner-section pt-page" id="home" style="background-image: url('img/background/home-bg.jpg')">

            <!--Particle Container-->
            <div id="particles-js"></div>

            <div class="banner-content">
                <!--Banner Text-->
                <h1 class="mb-20">Nuno <span>Facha</span></h1>

                <!--Animated Text-->
                <p class="cd-headline clip is-full-width">
                    <span>I am a </span>
                    <span class="cd-words-wrapper">
                            <b class="is-visible">Software Developer</b>
                            <b>Linux SysAdmin</b>
                        </span>
                </p>

            </div>

        </section>
        <!--Banner Section End-->


        <!--About Section Start-->
        <section class="about-section pt-page" id="about">
            <div class="section-container">
                <!--Page Heading-->
                <div class="page-heading">
                    <span class="icon"><i class="lnr lnr-user"></i></span>
                    <h2>About Me.</h2>
                </div>

                <!-- About Info Row Start-->
                <div class="row about mb-70">

                    <div class="col-lg-8">
                        <!--Personal Intro-->
                        <h3 class="mb-20">What about me?</h3>
                        <p>Apart from being a software developer, I enjoy most of my time being outdoors, mostly playing
                            Ingress, being a very active player in local, nationwide, and international Resistance
                            communities.</p>

                        <p>When forced indoors, I follow a number of sci-fi, drama and fantasy genre movies and
                            television shows, I spend a large amount of my free time exploring the latest technolgy
                            advancements in the development and server management world.</p>

                    </div>

                    <!--Personal Info-->
                    <div class="col-lg-4">
                        <div class="about-info">
                            <h3 class="mb-20">Personal Information</h3>
                            <ul>
                                <li><span class="title">Name</span><span class="value">Nuno Facha</span></li>
                                <li><span class="title">Age</span><span class="value">22 Years</span></li>
                                <li><span class="title">Residence</span><span class="value">Azores, Portugal</span></li>
                                <li><span class="title">Email</span><span class="value">eu@nunofacha.com</span></li>
                            </ul>
                            <!--                            <div class="resume-button mt-30">-->
                            <!--                                <a class="btn-main" href="#">Download Resume</a>-->
                            <!--                            </div>-->
                        </div>
                    </div>
                </div>
                <!-- About Info Row End-->

                <!--                &lt;!&ndash;Clients Row Start&ndash;&gt;-->
                <!--                <div class="row clients mb-70">-->
                <!--                    <div class="col-md-12">-->
                <!--                        <div class="subheading">-->
                <!--                            <h3>Clients</h3>-->
                <!--                        </div>-->
                <!--                    </div>-->

                <!--                    <div class="owl-carousel owl-theme">-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-01.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-02.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-03.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-04.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-05.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-06.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-07.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-08.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Client Logo&ndash;&gt;-->
                <!--                        <div class="client-logo">-->
                <!--                            <a href="#">-->
                <!--                                <img src="img/clients/client-09.png" alt="">-->
                <!--                            </a>-->
                <!--                        </div>-->

                <!--                    </div>-->
                <!--                </div>-->
                <!--                &lt;!&ndash;Clients Row End&ndash;&gt;-->

                <!--                &lt;!&ndash;Testimonials Row Start&ndash;&gt;-->
                <!--                <div class="row testimonials mb-50">-->
                <!--                    <div class="col-md-12">-->
                <!--                        <div class="subheading">-->
                <!--                            <h3>Testimonials</h3>-->
                <!--                        </div>-->
                <!--                        <div class="owl-carousel owl-theme">-->

                <!--                            &lt;!&ndash;Testimonail Item&ndash;&gt;-->
                <!--                            <div class="testimonial-item">-->
                <!--                                <div class="testimonial-content">-->
                <!--                                    <p>Ipsum ab necessitatibus numquam vitae quis. Nobis nostrum deserunt suscipit eos-->
                <!--                                        fugit. Consectetur dolorum temporibus facilis impedit exercitationem-->
                <!--                                        dignissimos.</p>-->
                <!--                                </div>-->
                <!--                                <div class="testimonial-meta">-->
                <!--                                    <img src="img/testimonials/author-1.jpg" alt="">-->
                <!--                                    <div class="meta-info">-->
                <!--                                        <h4>Kate Fox</h4>-->
                <!--                                        <p>Digital Marketing Executive</p>-->
                <!--                                    </div>-->
                <!--                                </div>-->
                <!--                            </div>-->

                <!--                            &lt;!&ndash;Testimonail Item&ndash;&gt;-->
                <!--                            <div class="testimonial-item">-->
                <!--                                <div class="testimonial-content">-->
                <!--                                    <p>Ipsum ab necessitatibus numquam vitae quis. Nobis nostrum deserunt suscipit eos-->
                <!--                                        fugit. Consectetur dolorum temporibus facilis impedit exercitationem-->
                <!--                                        dignissimos.</p>-->
                <!--                                </div>-->
                <!--                                <div class="testimonial-meta">-->
                <!--                                    <img src="img/testimonials/author-2.jpg" alt="">-->
                <!--                                    <div class="meta-info">-->
                <!--                                        <h4>Emma Jones</h4>-->
                <!--                                        <p>Creative Director</p>-->
                <!--                                    </div>-->
                <!--                                </div>-->
                <!--                            </div>-->

                <!--                            &lt;!&ndash;Testimonail Item&ndash;&gt;-->
                <!--                            <div class="testimonial-item">-->
                <!--                                <div class="testimonial-content">-->
                <!--                                    <p>Ipsum ab necessitatibus numquam vitae quis. Nobis nostrum deserunt suscipit eos-->
                <!--                                        fugit. Consectetur dolorum temporibus facilis impedit exercitationem-->
                <!--                                        dignissimos.</p>-->
                <!--                                </div>-->
                <!--                                <div class="testimonial-meta">-->
                <!--                                    <img src="img/testimonials/author-3.jpg" alt="">-->
                <!--                                    <div class="meta-info">-->
                <!--                                        <h4>Jack Smith</h4>-->
                <!--                                        <p>Marketing Director</p>-->
                <!--                                    </div>-->
                <!--                                </div>-->
                <!--                            </div>-->

                <!--                        </div>-->
                <!--                    </div>-->
                <!--                    &lt;!&ndash;Testimonials Row End&ndash;&gt;-->

                <!--                </div>-->
            </div>
        </section>
        <!--About Section Start-->


        <!--Resume Section Start-->
        <section class="resume-section pt-page" id="resume">
            <div class="section-container">
                <div class="page-heading">
                    <span class="icon"><i class="lnr lnr-license"></i></span>
                    <h2>My Resume.</h2>
                </div>

                <!--Education & Experience Row Start-->
                <div class="row mb-20">

                    <!--Experience Column Start-->
                    <div class="col-lg-6">
                        <div class="subheading">
                            <h3>Experience</h3>
                        </div>
                        <ul class="experience">

                            <!--Experience Item-->
                            <li>
                                <span class="line-left"></span>
                                <div class="content">
                                    <h4>Software Developer</h4>
                                    <h5>UrActive</h5>
                                    <p class="info">
                                        Software development using AngularJS and Java
                                        <br>
                                        Server and infrastructure management
                                        <br>
                                        Docker (Swarm) over Ubuntu
                                        <br>
                                        Continuous Integration (GitLab)
                                        <br>
                                        High Performance/High Availability services using NGINX and Docker

                                    </p>
                                </div>
                                <span class="year">
                                            <span class="to"><?= date( 'Y' ) ?></span>
                                            <span class="from">2020</span>
                                        </span>
                            </li>

                            <!--Experience Item-->
                            <li>
                                <span class="line-left"></span>
                                <div class="content">
                                    <h4>Founder / Software Developer</h4>
                                    <h5>FreeMcServer.net - Freelance</h5>
                                    <p class="info">
                                        Personal Project
                                        <br>
                                        Free Minecraft Server Hosting Service
                                        <br>
                                        The system is built using the php framework Yii2, MySQL as database service and
                                        relying on Docker.
                                    </p>
                                </div>
                                <span class="year">
                                            <span class="to"><?= date( 'Y' ) ?></span>
                                            <span class="from">2018</span>
                                        </span>
                            </li>

                            <!--Experience Item-->
                            <li>
                                <span class="line-left"></span>
                                <div class="content">
                                    <h4>Software Developer (Internship)</h4>
                                    <h5>Sparkyway</h5>
                                    <p class="info">
                                        Development and maintenance of web applications using the MVC Yii2 framework;
                                        <br>
                                        Configuration, maintenance and management of a Linux server infrastructure,
                                        mostly Ubuntu and CentosOS, using cPanel, Docker and HaProxy for load balancing;
                                        <br>
                                        Firewall configuration and management (mostly CSF) for the same servers;
                                        <br>
                                        Configuration of Docker images continuous integration system using Gitlab-CI;
                                    </p>
                                </div>
                                <span class="year">
                                            <span class="to">2019</span>
                                            <span class="from">2017</span>
                                        </span>
                            </li>

                            <!--Experience Item-->
                            <li>
                                <span class="line-left"></span>
                                <div class="content">
                                    <h4>Founder / Software Developer</h4>
                                    <h5>FreeTeamSpeak.pw - Freelance</h5>
                                    <p class="info">
                                        Personal Project
                                        <br>
                                        Free TeamSpeak Servers hosting provider
                                        <br>
                                        The project closed due to licensing changes that made it financially
                                        unsustainable.
                                        <br>
                                        The system was built using the PHP framework Yii2 relying on MySQL as a
                                        database.
                                    </p>
                                </div>
                                <span class="year">
                                            <span class="to">2018</span>
                                            <span class="from">2015</span>
                                        </span>
                            </li>

                        </ul>
                    </div>
                    <!--Experience Column End-->

                    <!--Education Column Start-->
                    <div class="col-lg-6">
                        <div class="subheading">
                            <h3>Education</h3>
                        </div>

                        <ul class="education">
                            <!--Education Item-->
                            <li>
                                <span class="line-left"></span>
                                <div class="content">
                                    <h4>Nginx Fundamentals: High Performance Servers</h4>
                                    <h5>Udemy (Ray Viljoen)</h5>
                                </div>
                                <span class="year">
                                            <span class="to">2020</span>
                                            <span class="from">2020</span>
                                        </span>
                            </li>
                            <!--Education Item-->
                            <li>
                                <span class="line-left"></span>
                                <div class="content">
                                    <h4>Computer Systems Management and Programming Technician</h4>
                                    <h5>Escola de Novas Tecnologias dos Açores</h5>
                                    <p class="info">
                                        Software Development
                                        <br>
                                        System and Network Management
                                    </p>
                                </div>
                                <span class="year">
                                            <span class="to">2017</span>
                                            <span class="from">2014</span>
                                        </span>
                            </li>

                            <!--Education Item-->
                            <li>
                                <span class="line-left"></span>
                                <div class="content">
                                    <h4>FCE</h4>
                                    <h5>Cambridge English</h5>
                                </div>
                                <span class="year">
                                            <span class="to">2014</span>
                                            <span class="from">2014</span>
                                        </span>
                            </li>


                        </ul>
                    </div>
                    <!--Education Column End-->

                </div>
                <!--Education & Experience Row End-->


                <!--Skills Row Start-->
                <!--                <div class="row">-->

                <!--                    &lt;!&ndash;Design Skills Column Start&ndash;&gt;-->
                <!--                    <div class="col-lg-6 skills">-->
                <!--                        <div class="subheading">-->
                <!--                            <h3>Design Skills</h3>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Design Skill Item&ndash;&gt;-->
                <!--                        <div class="skill-item">-->
                <!--                            <h4 class="progress-title">Coral Draw</h4>-->
                <!--                            <div class="progress">-->
                <!--                                <div class="progress-bar" data-progress-value="91">-->
                <!--                                    <div class="progress-value">91%</div>-->
                <!--                                </div>-->
                <!--                            </div>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Design Skill Item&ndash;&gt;-->
                <!--                        <div class="skill-item">-->
                <!--                            <h4 class="progress-title">Photoshop</h4>-->
                <!--                            <div class="progress">-->
                <!--                                <div class="progress-bar" data-progress-value="82">-->
                <!--                                    <div class="progress-value">82%</div>-->
                <!--                                </div>-->
                <!--                            </div>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Design Skill Item&ndash;&gt;-->
                <!--                        <div class="skill-item">-->
                <!--                            <h4 class="progress-title">Illustrator</h4>-->
                <!--                            <div class="progress">-->
                <!--                                <div class="progress-bar" data-progress-value="94">-->
                <!--                                    <div class="progress-value">94%</div>-->
                <!--                                </div>-->
                <!--                            </div>-->
                <!--                        </div>-->
                <!--                    </div>-->
                <!--                    &lt;!&ndash;Design Skills Column End&ndash;&gt;-->

                <!--                    &lt;!&ndash;Coding Skills Column Start&ndash;&gt;-->
                <!--                    <div class="col-lg-6 skills">-->
                <!--                        <div class="subheading">-->
                <!--                            <h3>Coding Skills</h3>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Coding Skill Item&ndash;&gt;-->
                <!--                        <div class="skill-item">-->
                <!--                            <h4 class="progress-title">HTML5</h4>-->
                <!--                            <div class="progress">-->
                <!--                                <div class="progress-bar" data-progress-value="83">-->
                <!--                                    <div class="progress-value">83%</div>-->
                <!--                                </div>-->
                <!--                            </div>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Coding Skill Item&ndash;&gt;-->
                <!--                        <div class="skill-item">-->
                <!--                            <h4 class="progress-title">CSS3</h4>-->
                <!--                            <div class="progress">-->
                <!--                                <div class="progress-bar" data-progress-value="75">-->
                <!--                                    <div class="progress-value">75%</div>-->
                <!--                                </div>-->
                <!--                            </div>-->
                <!--                        </div>-->

                <!--                        &lt;!&ndash;Coding Skill Item&ndash;&gt;-->
                <!--                        <div class="skill-item">-->
                <!--                            <h4 class="progress-title">Javascript</h4>-->
                <!--                            <div class="progress">-->
                <!--                                <div class="progress-bar" data-progress-value="92">-->
                <!--                                    <div class="progress-value">92%</div>-->
                <!--                                </div>-->
                <!--                            </div>-->
                <!--                        </div>-->

                <!--                    </div>-->
                <!--                    &lt;!&ndash;Coding Skills Column End&ndash;&gt;-->

                <!--                </div>-->
                <!--Skills Row End-->
            </div>
        </section>
        <!--Resume Section End-->

        <section class="about-section pt-page" id="events">
            <div class="section-container">
                <!--Page Heading-->
                <div class="page-heading">
                    <span class="icon"><i class="lnr lnr-user"></i></span>
                    <h2>Attended Events</h2>
                </div>
                <h3>WebSummit 2019 (Nov 2019)</h3>
                <br>
                <div class="row portfolio-items">
                    <div class="item col-md-3">
                        <a class="image-link" href="img/websummit-2019/IMG_20191106_162024.jpg">
                            <figure>
                                <img alt="" src="img/websummit-2019/IMG_20191106_162024-min.jpg">
                                <figcaption>
                                    <h4>WebSummit 2019</h4>
                                    <p>Lisbon, Portugal</p>
                                </figcaption>
                            </figure>
                        </a>
                    </div>
                    <div class="item col-md-3">
                        <a class="image-link" href="img/websummit-2019/IMG_20191107_111841.jpg">
                            <figure>
                                <img alt="" src="img/websummit-2019/IMG_20191107_111841-min.jpg">
                                <figcaption>
                                    <h4>WebSummit 2019</h4>
                                    <p>Lisbon, Portugal</p>
                                </figcaption>
                            </figure>
                        </a>
                    </div>
                    <div class="item col-md-3">
                        <a class="image-link" href="img/websummit-2019/IMG_20191107_163005.jpg">
                            <figure>
                                <img alt="" src="img/websummit-2019/IMG_20191107_163005-min.jpg">
                                <figcaption>
                                    <h4>WebSummit 2019</h4>
                                    <p>Lisbon, Portugal</p>
                                </figcaption>
                            </figure>
                        </a>
                    </div>
                    <div class="item col-md-3">
                        <a class="image-link" href="img/websummit-2019/IMG_20191107_170708.jpg">
                            <figure>
                                <img alt="" src="img/websummit-2019/IMG_20191107_170708-min.jpg">
                                <figcaption>
                                    <h4>WebSummit 2019</h4>
                                    <p>Lisbon, Portugal</p>
                                </figcaption>
                            </figure>
                        </a>
                    </div>
                </div>

            </div>
        </section>
        <section class="about-section pt-page" id="stats">
            <div class="section-container">
                <!--Page Heading-->
                <div class="page-heading">
                    <span class="icon"><i class="lnr lnr-code"></i></span>
                    <h2>Coding Stats</h2>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <figure>
                            <embed src="https://wakatime.com/share/@facha/2240dbbe-99f6-4e7b-b0dd-57a6b6093465.svg"></embed>
                        </figure>
                    </div>
                    <div class="col-md-6">
                        <figure>
                            <embed src="https://wakatime.com/share/@facha/eb4366eb-b62a-45b1-8041-cf02f50d8583.svg"></embed>
                        </figure>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-6">
                        <figure>
                            <embed src="https://wakatime.com/share/@facha/e42a5b2d-d1cd-4825-ae48-ca208c32e691.svg"></embed>
                        </figure>
                    </div>
                    <div class="col-md-6">
                        <figure>
                            <embed src="https://wakatime.com/share/@facha/4c6a357e-eaf0-451e-90c0-4e7b01e0b83d.svg"></embed>
                        </figure>
                    </div>
                </div>
            </div>

    </div>
    </section>
    <!--Porfolio Section Start-->
    <!--        <section id="portfolio" class="portfolio-section pt-page">-->
    <!--            <div class="section-container">-->

    <!--                &lt;!&ndash;Page Heading&ndash;&gt;-->
    <!--                <div class="page-heading">-->
    <!--                    <span class="icon"><i class="lnr lnr-briefcase"></i></span>-->
    <!--                    <h2>Portfolio.</h2>-->
    <!--                </div>-->

    <!--                <div class="row">-->
    <!--                    &lt;!&ndash;Portfolio Filter&ndash;&gt;-->
    <!--                    <div class="col-md-12 portfolio-filter text-center">-->
    <!--                        <ul>-->
    <!--                            <li class="active" data-filter="*">All</li>-->
    <!--                            <li data-filter=".brand">Brand</li>-->
    <!--                            <li data-filter=".design">Design</li>-->
    <!--                            <li data-filter=".graphic">Graphic</li>-->
    <!--                        </ul>-->
    <!--                    </div>-->
    <!--                </div>-->

    <!--                &lt;!&ndash;Portfolio Items&ndash;&gt;-->
    <!--                <div class="row portfolio-items mb-50">-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 brand graphic">-->
    <!--                        <a class="image-link" href="img/portfolio/img-1.jpg">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-1.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Book Design</h4>-->
    <!--                                    <p>Graphic</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 design">-->
    <!--                        <a class="ajax-link" href="single-work.html">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-2.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Notepad Design</h4>-->
    <!--                                    <p>Design</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 design graphic">-->
    <!--                        <a class="video-link" href="https://www.youtube.com/watch?v=k_okcNVZqqI">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-3.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Creative Cup</h4>-->
    <!--                                    <p>Graphic</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 design">-->
    <!--                        <a class="image-link" href="img/portfolio/img-4.jpg">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-4.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Business Card</h4>-->
    <!--                                    <p>Design</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 brand">-->
    <!--                        <a class="ajax-link" href="single-work.html">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-5.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Back Pack</h4>-->
    <!--                                    <p>Brand</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 design">-->
    <!--                        <a class="ajax-link" href="single-work.html">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-6.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Door Hangar</h4>-->
    <!--                                    <p>Design</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 graphic">-->
    <!--                        <a class="image-link" href="img/portfolio/img-7.jpg">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-7.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Notepad Design</h4>-->
    <!--                                    <p>graphic</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 brand">-->
    <!--                        <a class="image-link" href="img/portfolio/img-8.jpg">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-8.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Creative Flyer</h4>-->
    <!--                                    <p>Brand</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 brand">-->
    <!--                        <a class="video-link" href="https://www.youtube.com/watch?v=k_okcNVZqqI">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-9.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Mobile App</h4>-->
    <!--                                    <p>Brand</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 graphic">-->
    <!--                        <a class="image-link" href="img/portfolio/img-10.jpg">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-10.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Cup Design</h4>-->
    <!--                                    <p>Graphic</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Portfolio Item&ndash;&gt;-->
    <!--                    <div class="item col-lg-4 col-sm-6 brand graphic">-->
    <!--                        <a class="ajax-link" href="single-work.html">-->
    <!--                            <figure>-->
    <!--                                <img src="img/portfolio/img-11.jpg" alt="">-->
    <!--                                <figcaption>-->
    <!--                                    <h4>Creative Box</h4>-->
    <!--                                    <p>brand</p>-->
    <!--                                </figcaption>-->
    <!--                            </figure>-->
    <!--                        </a>-->
    <!--                    </div>-->

    <!--                </div>-->
    <!--            </div>-->
    <!--        </section>-->
    <!--Porfolio Section End-->


    <!--Contact Section Start-->
    <!--        <section id="contact" class="contact-section pt-page">-->
    <!--            <div class="section-container">-->

    <!--                &lt;!&ndash;Page Heading&ndash;&gt;-->
    <!--                <div class="page-heading">-->
    <!--                    <span class="icon"><i class="lnr lnr-envelope"></i></span>-->
    <!--                    <h2>Contact Me.</h2>-->
    <!--                </div>-->

    <!--                &lt;!&ndash;Form Row&ndash;&gt;-->
    <!--                <div class="row mb-70">-->
    <!--                    <div class="col-lg-8  offset-lg-2">-->
    <!--                        <div class="subheading">-->
    <!--                            <h3>Let's Talk</h3>-->
    <!--                        </div>-->

    <!--                        &lt;!&ndash;Form Start&ndash;&gt;-->
    <!--                        <form id="contact-form" method="post" action="mail.php">-->
    <!--                            <div class="row">-->


    <!--                                &lt;!&ndash;Name Field&ndash;&gt;-->
    <!--                                <div class="col-md-6 mb-50">-->
    <!--                                            <span class="input">-->
    <!--                                                <input class="input__field cf-validate" type="text" id="cf-name"-->
    <!--                                                       name="name"/>-->
    <!--                                                <label class="input__label" for="cf-name">Name</label>-->
    <!--                                            </span>-->
    <!--                                </div>-->

    <!--                                &lt;!&ndash;Email Field&ndash;&gt;-->
    <!--                                <div class="col-md-6 mb-50">-->
    <!--                                            <span class="input">-->
    <!--                                                <input class="input__field cf-validate" type="text" id="cf-email"-->
    <!--                                                       name="email"/>-->
    <!--                                                <label class="input__label" for="cf-email">Email</label>-->
    <!--                                            </span>-->
    <!--                                </div>-->

    <!--                                &lt;!&ndash;Message Box&ndash;&gt;-->
    <!--                                <div class="col-md-12 mb-30">-->
    <!--                                            <span class="input">-->
    <!--                                                <textarea class="input__field cf-validate" id="cf-message"-->
    <!--                                                          name="message" rows="5"></textarea>-->
    <!--                                                <label class="input__label"-->
    <!--                                                       for="cf-message">How can we help you?</label>-->
    <!--                                            </span>-->
    <!--                                </div>-->

    <!--                                <div class="alert-container col-md-12"></div>-->

    <!--                                &lt;!&ndash;Submit Button&ndash;&gt;-->
    <!--                                <div class="col-md-12 text-center">-->
    <!--                                    <button id="cf-submit" class="btn-main">Send Message</button>-->
    <!--                                </div>-->


    <!--                            </div>-->
    <!--                        </form>-->
    <!--                        &lt;!&ndash;Form End&ndash;&gt;-->

    <!--                    </div>-->
    <!--                </div>-->

    <!--                &lt;!&ndash;Contact Info Row Start&ndash;&gt;-->
    <!--                <div class="row contact-info mb-70">-->

    <!--                    &lt;!&ndash;Contact Info Item&ndash;&gt;-->
    <!--                    <div class="col-md-4 info-item">-->
    <!--                        <span class="icon"><i class="fas fa-paper-plane"></i></span>-->
    <!--                        <h5><a href="mailto:example@example.com">example@example.com</a></h5>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Contact Info Item&ndash;&gt;-->
    <!--                    <div class="col-md-4 info-item">-->
    <!--                        <span class="icon"><i class="fas fa-map-marker-alt"></i></span>-->
    <!--                        <h5>123 Lorem Ipsum, USA</h5>-->
    <!--                    </div>-->

    <!--                    &lt;!&ndash;Contact Info Item&ndash;&gt;-->
    <!--                    <div class="col-md-4 info-item">-->
    <!--                        <span class="icon"><i class="fas fa-phone"></i></span>-->
    <!--                        <h5>(+1) 123 456 7890</h5>-->
    <!--                    </div>-->

    <!--                </div>-->
    <!--                &lt;!&ndash;Contact Info Row End&ndash;&gt;-->

    <!--            </div>-->
    <!--            &lt;!&ndash;Google Map Start&ndash;&gt;-->
    <!--            <div class="google-map">-->
    <!--                <div id="map" data-latitude="-37.817214" data-longitude="144.955925" data-zoom="15"></div>-->
    <!--            </div>-->
    <!--            &lt;!&ndash;Google Map End&ndash;&gt;-->
    <!--        </section>-->
    <!--Contact Section End-->


</div>

<!--Main End-->
</div>

<!--Ajax Portfolio Container Start-->
<div class="ajax-portfolio-popup" data-simplebar>
    <span class="ajax-loader"></span>
    <div class="navigation-wrap">
        <span class="popup-close"><i class="fas fa-times-circle"></i></span>
    </div>
    <div class="content-wrap">
        <div class="popup-content"></div>
    </div>
</div>
<!--Ajax Portfolio Container End-->

<!--Jquery JS-->
<script src="js/jquery.min.js"></script>
<!--Plugins JS-->
<script src="js/plugins.min.js"></script>
<!--Google Map Api-->
<script src="https://maps.google.com/maps/api/js?sensor=false"></script>
<!--Site Main JS-->
<script src="js/main.js"></script>
<script>
    //Particles
    particlesJS.load('particles-js', 'js/particles2.json', function () {
        console.log('callback - particles.js config loaded');
    });
</script>
</body>
</html>


